...
// The adjusted interface for comments from the database
interface Comment {
  id: string;
  post_id: string;
  author_name: string; // instead of name
  author_email: string; // instead of email
  content: string;
  created_at: string;
  is_approved: boolean;
  avatar?: string | null;
...
export const PostComments: React.FC<PostCommentsProps> = ({ postId }) => {
    ...
    const fetchComments = async () => {
      try {
        const { data, error } = await supabaseClient
          .from('comments') // Corrected table name
          .select('*')
          .eq('post_id', postId)
          .eq('is_approved', true)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Submit comment to database with adjusted field names
      const { error } = await supabaseClient.from('comments').insert([
        {
          post_id: postId,
          author_name: newComment.name, // name → author_name
          author_email: newComment.email, // email → author_email
