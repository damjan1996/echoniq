...
interface SimpleStreamingLinksProps {
  ...
  beatport: string | null | undefined;
...
export default function ArtistDetailPage({ artist }: ArtistDetailPageProps) {
  ...
  ].filter((link) => link.url); // Filtere Links ohne URL

  const openGraph = generateArtistOpenGraphMetadata({
    name: artist.name,
    bio: artist.bio === null ? undefined : artist.bio,
    profileImage: artist.profile_image === null ? undefined : artist.profile_image,
    slug: artist.slug,
  });
                ...
                <div className="mb-8">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {socialLinks.map((link) => (
                      <a
                        key={link.name}
                        href={link.url as string} // Typumwandlung, da wir durch Filter sichergestellt haben, dass url existiert
                        target="_blank"
                        rel="noopener noreferrer"
